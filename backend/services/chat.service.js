...
class ChatService {
  ...
  async createChat(userId, members) {
    ...
    if (existingChat) {
      throw createConflictError('Chat already exists')
    }

    // Generate chat name
    let name = 'New Chat'
    if (chatMembers.length > 2) {
      const users = await userModel.find({ _id: { $in: chatMembers } }).select('name')
  ...
  async processChatInfo(chat, userId) {
    ...
    if (lastMessage) {
      ...
      modifiedChat.lastMessage = lastMessage.type === 'image' ? '📷 Image' : lastMessage.content
      modifiedChat.isMyMessage = lastMessage.senderId?.toString() === userId.toString()
  ...
  async addMembersToChat(chatId, userId, newMembers) {
    ...
    const membersToAdd = newMembers
      ...
      .filter(member => !chat.members.map(member => member.toString()).includes(member.toString()))
    ...
    const addedMembers = await userModel.find({ _id: { $in: membersToAdd } }).select('name')
  ...
  async removeMemberFromChat(chatId, targetId, userId) {
    ...
    chat.members = chat.members.filter(member => member.toString() !== targetId.toString())

    await chat.save()

    // Create system message
    const user = await userModel.findById(targetId).select('name')
    const actionUser = userId ? await userModel.findById(userId).select('name') : null
  ...
  async editChat(chatId, userId, updateData) {
    ...
    // Create system message
    const user = await userModel.findById(userId).select('name')
    await chatHistoryService.createSystemMessage(chat, `${user.name} changed chat info`)
